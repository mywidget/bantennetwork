$(document).on('click', '.page-link', function(e){
	...
	$.post(base_url+'akun/cari/hitungan'+ current_query, {'search':search,'page-number' : page_number}, function(data){
		$('#all-hitungan').html(data);
		}).done(function() {
		NProgress.start();
		}).always(function() {
		NProgress.done();
...
function load_data(search,nomor){
	...
	$.post(base_url+'akun/cari/hitungan', {'search':search,'page-number' : nomor},function(data){
		$('#all-hitungan').html(data);
		}).done(function() {
		NProgress.start();　
		}).always(function() {
...
$(document).on('submit', '#filter-form', function(e){
	...
	var form = $(this);
	// console.log(form);
	$.post(base_url+'akun/cari/hitungan', $(form).serialize(), function(data){
		$('#all-hitungan').html(data);
		}).done(function() {
		NProgress.start();　
		}).always(function() {
...
function hitungan(id)
{
	...
	if(id){
		// console.log('edit');
		$("#btn-hitungan").html("Update");
		var search = $('#search').val();
		$.ajax({
			type: "POST",
			url: base_url+"akun/crud",
			dataType: 'json',
			data: {view:'hitungan',id:id,type:"get"},
			beforeSend: function () {
				$('.se-pre-con').fadeIn();
			},
