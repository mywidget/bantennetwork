<?php
	...
	class News extends CI_Controller {
		...
		{
			$data['title'] = 'News | lenteranews.tv';
            ...
            "@type" =>  "Organization",
            "name" =>  "Lentera News",
            "url" =>  "https://www.lenternews.tv/news",
            "sameAs" => [
			...
			if($qry->num_rows()){
				...
				$data['pilihan'] =  $this->model_app->view_join_where('posting','cat','id_cat',['posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s"),'status'=>3],'tanggal','desc',1);
				$data['video'] =  $this->model_app->view_join_where('posting','cat','id_cat',['posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s"),'youtube'=>''],'tanggal','desc',3);
				...
				$config['base_url']    = base_url('news/newsPagination/'); 
				...
				// Get records posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s")
				$conditions = array( 
				'limit' => $this->perPage 
				); 
				$conditions['where'] = array(
