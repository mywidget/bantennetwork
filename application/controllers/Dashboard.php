<?php
    ...
    class Dashboard extends CI_Controller
    {
        ...
        {
            $where = ['posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s"),'`posting`.`publish`' => 'Y','`posting`.`status`' => '2'];
            ...
            $where5 = ['posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s"),'`posting`.`publish`' => 'Y','`posting`.`status`' => '4'];
            ...
            'headline'=>$this->model_app->view_join_where('posting','cat','id_cat',$where,'tanggal','desc',5),
            'terbaru'=>$this->model_app->view_join_where('posting','cat','id_cat',$where2,'tanggal','desc',10),
            'sorotan'=>$this->model_app->view_join_where('posting','cat','id_cat',$where3,'tanggal','desc',3),
