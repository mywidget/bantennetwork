<?php
	...
	class Artikel extends CI_Controller {
		...
		{
			$data['title'] = 'Artikel | lenternews.tv';
            ...
            "@type" =>  "Organization",
            "name" =>  "Lentera News",
            "url" =>  "https://www.lenternews.tv/artikel",
            "sameAs" => [
			...
			if($qry->num_rows()){
				...
				$where3 = ['posting.tanggal <'=>mdate("%Y-%m-%d %H:%i:%s"),'`posting`.`publish`' => 'Y','`posting`.`status`' => '1'];
				...
				$data['description'] = tag_key('site_desc');
				$data['keywords'] = tag_key('site_desc');
				$data['canonical']=base_url('artikel');
				$data['url_image'] = base_url('assets/thumb.jpg');
				$data['headline'] =  $this->model_app->view_join_where('posting','cat','id_cat',$where,'tanggal','desc',5);
				$data['terbaru'] =  $this->model_app->view_join_where('posting','cat','id_cat',$where2,'tanggal','desc',8);
				$data['sorotan'] = $this->model_app->view_join_where('posting','cat','id_cat',$where3,'tanggal','desc',3);
				...
				$config['base_url']    = base_url('artikel/Pagination/'); 
				...
				$conditions['where'] = array(
				'kategori_seo' => 'artikel',
				'posting.tanggal <' => mdate("%Y-%m-%d %H:%i:%s")
				);
				$data['posts'] = $this->model_data->getProgram($conditions); 
				//end
				$this->template->load(template().'/themes',template().'/artikel',$data);
				}else{
				$data = error_page();
				$this->template->load(template().'/themes',template().'/404',$data);
			}
            // $this->template->load(template().'/themes',template().'/news',$data);
		}
		function Pagination(){ 
			// Define offset 
