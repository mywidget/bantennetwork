<?php 
	function descHome($str=''){
		$ci = & get_instance();
		if($str=='desc'){
			$querys=$ci->db->query("SELECT deskripsi FROM `posting` WHERE publish='Y' order by tanggal desc limit 1");
			$data = tag_key('site_desc');
			...
			}elseif($str=='site_keys'){
			$querys=$ci->db->query("SELECT kata_kunci FROM `posting` WHERE publish='Y' order by tanggal desc limit 1");
			$data = tag_key('site_keys');
			if($querys->num_rows() >0){
				$row = $querys->row();
	...
	function imgDetail($str=''){
		$ci = & get_instance();
		$querys=$ci->db->query("SELECT folder,gambar FROM `posting` WHERE id_post='$str' AND publish='Y'");			
